...
微尘咨询平台是一个集社区交流、即时通讯、智能助手于一体的 Web 应用。用户可注册账号、发布帖子、评论互动，也可进入三个固定主题的聊天室进行交流（采用 AJAX 轮询实现准实时更新），或使用 AI 助手获取自动回复（基于 Ollama 本地模型）。管理员具备封禁用户、管理帖子与反馈等后台功能，并可利用 AI 整理邮件。系统部署时通过 ngrok 内网穿透实现公网访问。
...
- 聊天室：三个固定聊天室（使用 AJAX 轮询方式实现准实时消息更新）
- AI 助手：
...
| 实时通信 | AJAX 轮询（JavaScript + 后端） |
| AI 集成 | Ollama 本地模型，JSON 数据交互 |
| 前端 | jQuery、Bootstrap、自定义 CSS |
| 测试工具 | Python + Selenium + unittest 自动化测试 |
...
│   │   └── ...
│   └── 测试代码/              # Python + Selenium + unittest 测试脚本（高展延编写）
├── 8 项目答辩/                # 答辩 PPT
└── 9 项目总结/                # 项目总结报告
```
...
| 成员 | 角色 | 负责模块 |
|------|------|----------|
| 胡孜恒 | 项目组长 | 所有项目安全性检查、登录模块、个人中心（修改密码/信息）、聊天室模块（AJAX 轮询实现）、AI助手模块（Ollama 集成）、用户反馈模块、管理端全部功能、项目部署 |
| 高展延 | 组员 | 全部代码的测试（使用 Python + Selenium + unittest 编写自动化测试脚本） |
...
- Ollama（需本地安装deepssk-R1:8B并拉取对话模型，用于 AI 助手）
- IIS（用于本地部署，可选）
- ngrok（用于内网穿透，可选）
- Python 环境（如需运行测试，请安装 Python 3.x、Selenium 及对应浏览器驱动）
...
1. 安装 Python 依赖（根据测试代码中的导入确定，通常包含 Selenium）：
   ```bash
   pip install selenium
   ```
2. 下载并配置对应浏览器的 WebDriver（如 ChromeDriver），确保其路径在环境变量中。
3. 运行测试脚本（具体命令参照 `测试代码` 目录下的入口文件）。
...
- 聊天室实现：采用 AJAX 轮询方式实现准实时消息更新，在课程项目场景下是一种务实的解决方案。未来可升级为 WebSocket 或 SignalR，以获得真正的实时推送能力。
- AI 集成：主动引入 Ollama 本地大模型，实现了用户端智能对话和管理端邮件整理，为大二项目中的亮点技术探索。
- 测试体系：由高展延编写的 Python 测试脚本使用了 Selenium + unittest 框架，实现了基于浏览器的自动化功能测试，覆盖主要用户流程。
