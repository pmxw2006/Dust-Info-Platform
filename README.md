...
微尘咨询平台是一个集社区交流、即时通讯、智能助手于一体的 Web 应用。用户可注册账号、发布帖子、评论互动，也可进入三个固定主题的聊天室进行实时交流，或使用 AI 助手获取自动回复。管理员具备封禁用户、管理帖子与反馈等后台功能。系统部署时通过 ngrok 内网穿透实现公网访问。
...
- 聊天室：三个固定聊天室（基于 JavaScript + 后端实时通信，WebSocket 实现）
- AI 助手：提供自动问答交互
- 消息模块：站内消息通知
- 用户反馈：用户可提交反馈信息
- 管理后台（预设管理员账号）：
...
│   │   ├── Views/             # 视图页面（.cshtml）
│   │   ├── Services/          # 服务层（如 LiaoTianService 等）
...
| 成员 | 角色 | 负责模块 |
|------|------|----------|
| 胡孜恒 | 项目组长 | 所有项目安全性检查、登录模块、个人中心（修改密码/信息）、聊天室模块、AI助手模块、用户反馈模块、管理端全部功能 |
...
4. 使用 Visual Studio 打开 `6 项目源码/WebApplication1/WebApplication1.sln`。

5. 按 F5 运行项目，系统将自动编译并在浏览器中打开主页。
...
- 架构模式：项目未强制采用 BLL/DAL/Mods 分层，而是遵循 ASP.NET MVC 约定，配合 Services 处理业务逻辑，结构清晰。
...
- 可提升点：部分控制器代码较长，可进一步提取业务逻辑；前端页面响应式适配可优化；可增加 API 速率限制防止滥用。
